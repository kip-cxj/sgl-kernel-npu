...
Buffer::get_dispatch_layout(const torch::Tensor& topk_idx, int num_experts,
    ...
    auto num_tokens_per_expert = at::empty({num_experts}, x.options());
    auto num_tokens_per_rank = at::empty({num_ranks}, x.options());
    auto is_token_in_rank = at::empty({num_tokens, num_ranks}, x.options());
