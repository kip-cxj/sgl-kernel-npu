...
#include "../kernels/dispatch_layout_tiling.h"
...
{
    OP_LOGD(nodeName, "numToken is %u.", tilingData.dispatchLayoutInfo.numToken);
    ...
    DispatchLayoutTilingData &tilingData)
{
    ...
    auto numTokensPtr = attrs->GetAttrPointer<char>(static_cast<int>(ATTR_NUM_TOKENS_INDEX));
    auto numRanksPtr = attrs->GetAttrPointer<char>(static_cast<int>(ATTR_NUM_RANKS_INDEX));
    auto numExpertsPtr = attrs->GetAttrPointer<int64_t>(ATTR_NUM_EXPERTS_INDEX);
    auto numTopkPtr = attrs->GetAttrPointer<char>(static_cast<int>(ATTR_NUM_TOPK_INDEX));
    ...
    OP_TILING_CHECK((*numTopkPtr <= 0) || (*numTopkPtr > K_MAX),
        OP_LOGE(nodeName, "numTopkPtr is invalid, only support (0, %ld], but got numTopk=%ld.", K_MAX, *numTopkPtr),
        return ge::GRAPH_FAILED);
...
static bool CheckTensorShape(gert::TilingContext *context, const char *nodeName)
{
    ...
    int64_t topkIdxDim0 = topkIdxStorageShape->GetStroageShape().GetDim(0);
    int64_t topkIdxDim1 = topkIdxStorageShape->GetStroageShape().GetDim(1);
    
    OP_TILING_CHECK((topkIdxStorageShape->GetStroageShape().GetDimNum() != TWO_DIMS),
        OP_LOGE(nodeName, "topkIdx must be 2-dimension, but get %lu dim.",
            topkIdxStorageShape->GetStroageShape().GetDimNum()), return false);
